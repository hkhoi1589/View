...
Modal.TextArea = function TextArea({ children, id, text, setText, resize }) {
	return (
		<textarea
			name='postContent'
			id={id}
...
Modal.Preview = function Preview({ selectedImage, setSelectedImage }) {
	...
	if (selectedImage instanceof File) {
		file = URL.createObjectURL(selectedImage); // tao img object url
	} else file = selectedImage;
...
Modal.UploadFile = function UploadFile({ children, id, setSelectedImage }) {
	...
	const imageChange = (e) => {
		...
		if (e.target.files && e.target.files.length > 0 && /image/.test(e.target.files[0].type)) {
			...
			dispatch(handleAlert({ message: 'Only accept images', type: 'error', isAlert: true }));
		}
	};
	return (
