...
import { handleAlert } from './uiReducer';
...
export const postReducer = createSlice({
	...
	reducers: {
		...
		SET_ADDING_POST: (state, action) => {
			return {
				...state,
				isAddingPost: action.payload,
			};
		},
		SET_ADDING_CMT: (state, action) => {
			return {
				...state,
				isAddingCmt: action.payload,
			};
		},
		...
		INCREASE_POST_LIKES: (state, action) => {
			return {
				...state,
				posts: state.posts.map((post) => {
					if (post._id === action.payload._id) {
				...
				currPost: !isEmpty(state.currPost)
					? {
							...state.currPost,
							likers: [...state.currPost.likers, action.payload.likers],
					  }
					: state.currPost,
			};
		},
		DECREASE_POST_LIKES: (state, action) => {
			return {
				...state,
				posts: state.posts.map((post) => {
					if (post._id === action.payload._id) {
						return {
							...post,
							likers: post.likers.filter((_id) => _id !== action.payload.likers),
						};
					}
					return post;
				}),
				currPost: !isEmpty(state.currPost)
					? {
							...state.currPost,
							likers: state.currPost.likers.filter(
								(_id) => _id !== action.payload.likers
							),
					  }
					: state.currPost,
			};
...
export const GetAllPosts = (page) => async (dispatch) => {
	try {
		if (page > homePage) {
			...
			if (responseOK && page !== 1 && postsResult.data.length !== 0) {
				// add more posts
				// neu thanh cong
...
export const GetUserPosts = (authorId, page) => async (dispatch) => {
	try {
		if (page > userPage) {
			...
			if (responseOK && page !== 1 && userPostsResult.data.length !== 0) {
				// neu thanh cong
...
export const LikePost = (userId, id) => async (dispatch) => {
	await dispatch(INCREASE_POST_LIKES({ _id: id, likers: userId }));
	try {
...
export const DislikePost = (userId, id) => async (dispatch) => {
	await dispatch(DECREASE_POST_LIKES({ _id: id, likers: userId }));
	try {
